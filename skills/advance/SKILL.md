...
Before reusing evidence, inspect recorded file hashes and `effective_verification`. Record `mark-delivery-change` for behavior- or dependency-affecting work before new verification, which invalidates affected current evidence and acceptance. If a review requests a correction under existing scope, use `prepare-correction` on that increment, then start it again. Stop at a real blocker, necessary product decision, or a delivery ready for review.
