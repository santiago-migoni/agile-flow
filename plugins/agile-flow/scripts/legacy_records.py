    ...
    def validate(self, state: dict[str, Any]) -> None:
        ...
        for key, prefix in (("backlog", "ITEM-"), ("increments", "INC-"), ("decisions", "DEC-"),
            ...
            for item in state.get(key, []):
                ...
                if not isinstance(identifier, str) or not identifier.startswith(("ITEM-", "US-", "NFR-", "BUG-", "TECH-", "SPIKE-") if key == "backlog" else prefix) or identifier in ids:
...
def new_id(state: dict[str, Any], collection: str, prefix: str) -> str:
    numbers = [int(row["id"].split("-")[-1]) for row in state[collection] if row["id"].startswith(prefix + "-") and row["id"].split("-")[-1].isdigit()]
