...
When consolidating an existing release, keep each BL's bounded contribution and rationale in its Planned scope table. Reconcile alternative applicability and dependent behavior after a decision. Keep executive context concise by referencing principal agreements; retain original quotations and historical records. Additional context is a lossless fallback for unknown fields, not the intended home for core delivery scope.
